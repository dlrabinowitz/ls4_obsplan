...
main(int argc, char **argv)
{
    ...
    FILE *input;
    char string[1024];
    char tesselation_file[1024];
    int n_exclude,exclude_list[MAX_EXCLUDES];
    int repeat_flag=0; /* set to 1 to repeat each pair of fields with offset */

    if(argc!=12){
        fprintf(stderr,"syntax: get_fields ra1 ra2 d1 d2 exptime interval iterations obs_code excluded_list offset_flag tesselation_file\n");
        exit(-1);
    }
    ...
    strcpy(tesselation_file,argv[11]);

    input=fopen(tesselation_file,"r");
    if(input==NULL){
